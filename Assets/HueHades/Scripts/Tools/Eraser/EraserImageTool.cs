    ...
    public class EraserImageTool : ImageTool
    {
        ...
        protected override void OnUseUpdate(Vector2 currentPoint, float currentPressure, float currentTilt)
        {
            ...
            if (paintPoints.Count > 0)
            {
                ...
                {
                    if (i == paintPoints.Count - 1) TextureDebugger.BeginSession("Eraser Image Tool Paint"); ;
                ...
                RenderTextureUtilities.LayerImageArea(_layerCopyBuffer, _layerOperatingCopyBuffer, bufferStartX, bufferStartY, bufferWidth, bufferHeight, _paintBuffer, Common.ColorBlendMode.Erase, bufferStartX, bufferStartY, _paintCanvas.TileMode, _paintCanvas.TileMode, _toolContext.BrushPreset.color.a);
